...
The exit code equals the number of Pact files with errors (not the number of errors in one comparison).`,
  ...
  .action(async (pactPaths: string[], options: { oas?: string; asyncapi?: string }) => {
    if (!options.oas && !options.asyncapi) {
      console.error("Error: at least one of --oas or --asyncapi must be provided");
      process.exit(1);
    }
    const runner = new Runner();
    const exitCode = await runner.run({ oasPath: options.oas, asyncapiPath: options.asyncapi }, pactPaths);
